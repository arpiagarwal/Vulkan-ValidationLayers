...
TEST_F(VkLayerTest, InvalidRenderPassEndFragmentDensityMapOffsetQCOM) {
    ...
    std::vector<VkAttachmentReference2> fdm = {
        {VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2, nullptr, 0, VK_IMAGE_LAYOUT_GENERAL, 0},
    };

    std::vector<VkAttachmentReference2> input = {
        {VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2, nullptr, 1, VK_IMAGE_LAYOUT_GENERAL, 0},
    };

    std::vector<VkAttachmentReference2> color = {
        {VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2, nullptr, 2, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, 0},
        {VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2, nullptr, 3, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, 0},
    };
    ...
    std::vector<VkAttachmentReference2> resolve = {
        {VK_STRUCTURE_TYPE_ATTACHMENT_REFERENCE_2, nullptr, 5, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, 0},
    ...
    vk::CreateImageView(m_device->device(), &ivci, NULL, &iv[2]);

    ivci.image = color_image2.handle();
    vk::CreateImageView(m_device->device(), &ivci, NULL, &iv[3]);

    ivci.format = VK_FORMAT_D16_UNORM;
    ivci.image = depth_image.handle();
    vk::CreateImageView(m_device->device(), &ivci, NULL, &iv[4]);

    ivci.format = VK_FORMAT_R8G8B8A8_UNORM;
    ivci.image = resolve_image.handle();
    vk::CreateImageView(m_device->device(), &ivci, NULL, &iv[5]);

    ivci.image = preserve_image.handle();
    vk::CreateImageView(m_device->device(), &ivci, NULL, &iv[6]);
    ASSERT_VK_SUCCESS(err);
    ...
    VkFramebuffer fb1;
    VkFramebuffer fb2;
    err = vk::CreateFramebuffer(m_device->device(), &fbci, nullptr, &fb1);
    ASSERT_VK_SUCCESS(err);

    fbci.renderPass = rp2[1];
    err = vk::CreateFramebuffer(m_device->device(), &fbci, nullptr, &fb2);
    ASSERT_VK_SUCCESS(err);

    // define renderpass begin info
    VkRenderPassBeginInfo rpbi1 = {VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr, rp2[0], fb1, {{0, 0}, {16, 16}}, 0, nullptr};
    VkRenderPassBeginInfo rpbi2 = {VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr, rp2[1], fb2, {{0, 0}, {16, 16}}, 0, nullptr};

    if (rp2Supported) {
        VkSubpassFragmentDensityMapOffsetEndInfoQCOM offsetting = {};
